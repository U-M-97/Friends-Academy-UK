...
const Video = () => {
  ...
  const getVideos = async () => {
    const files = await axios.get(`${process.env.url}/videosClient`, {
      params: { id: user._id },
    });
  ...
  const sendAccessReq = async () => {
    setLoading(true);
    setMessage("");
    const res = await axios.post(`${process.env.url}/videosClient`, {
      id: user._id,
    });
    ...
    <div className="flex items-center font-main justify-center">
      ...
      {access === true ? (
          ...
          <div className="flex flex-col h-accommodation overflow-y-auto px-2">
            {videos &&
              videos.map((video) => {
                return (
                  <div
                    className={`border border-pink rounded-md flex items-center px-2 py-1 cursor-pointer my-1 ${
                      selectedVideo === video.url ? `bg-pink text-white` : null
                    }`}
                    key={video}
        ...
        <div className="flex flex-col h-96 items-center justify-center">
          <p className="text-rose-600 font-bold text-4xl">
            You dont have access to Videos
          </p>
          {loading === false ? (
            <button
              className="mt-10 text-4xl bg-green w-96 py-3 rounded-md font-bold"
              onClick={sendAccessReq}
            >
              Send Access Request
            </button>
          ) : (
            <div className="mt-10 text-4xl bg-green flex items-center justify-center w-96 py-3 rounded-md font-bold">
              <CircularProgress sx={{ color: "#e75387" }} />
            </div>
          )}
          {message !== "" ? (
            <p
              className={`text-2xl mt-6 px-10 py-2 font-bold ${
