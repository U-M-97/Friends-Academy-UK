...
const MakeAPayment = () => {
  ...
  const handleVerify = async () => {
    setReqApi(true);
    try {
      const res = await axios.get(`${process.env.url}/clientRoomBooking`, {
        params: { id: bookingId },
      });
  ...
  return (
    <div className="h-96 mt-10 font-main">
      <div className="flex flex-col items-center justify-center ">
        ...
        <input
          className="px-5 py-3 rounded-md mt-5 w-96 border"
          onChange={(e) => setBookingId(e.target.value)}
        />
        {reqApi === false && verifyButton === true ? (
          <button
            className="mt-5 bg-green font-bold w-40 py-2 rounded-md text-xl hover:bg-greenHover"
        ...
        {bookingVerified === true ? (
          <div className="mt-2 flex flex-col items-center justify-center">
            ...
            <button
              className="text-2xl mt-5 bg-green px-5 py-2 font-bold rounded-md hover:bg-greenHover"
              onClick={handlePay}
            >
              Pay £{payment}
            </button>
