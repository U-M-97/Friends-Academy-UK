  ...
  if (tokenCheck === "Allowed") {
    if (req.method === "POST") {
      ...
      if (req.body.reqMethod === "Delete Booking") {
        ...
        const deleteMember = await Room.findOneAndUpdate(
          { _id: roomId },
          {
            $pull: {
              roomMembers: {
      ...
      try {
        ...
        if (reqMethod === "Add Member") {
            ...
            try {
              if (room) {
                const isBooked = room.roomMembers.find((member) => {
                  console.log(
                    dayjs(member.checkIn).isBetween(
                      dayjs(checkIn),
                      dayjs(checkOut),
                    ),
                  );
                  if (
                    ...
                    dayjs(member.checkOut).isBetween(
                      dayjs(checkIn),
                      dayjs(checkOut),
                    ) ||
                    dayjs(checkIn).isBetween(
                      dayjs(member.checkIn),
                      dayjs(member.checkOut),
                    ) ||
                    dayjs(checkOut).isBetween(
                      dayjs(member.checkIn),
                      dayjs(member.checkOut),
                    ) ||
                    dayjs(checkIn).isSame(dayjs(member.checkIn)) ||
                    dayjs(checkIn).isSame(dayjs(member.checkOut)) ||
                    dayjs(checkOut).isSame(dayjs(member.checkIn)) ||
                    dayjs(checkOut).isSame(dayjs(member.checkOut))
                  ) {
                    return member;
                  }
                });
                  ...
                  if (email.length !== 0) {
                    ...
                    let mailOptions = {
                      from: {
                        ...
                        address: "team@friendsacademy.co.uk",
                      },
                      to: email,
                      subject: "Room Booking",
                      text: `Your Room is booked successfully. You can pay now by adding your booking ID ${
                        updateRoom.roomMembers[
                          updateRoom.roomMembers.length - 1
                        ]._id
                      } in Make a Payment option on our Website`,
                      html: `
                                                <p style="font-size: medium">Your Room is booked successfully. You can pay now by adding your booking ID</p> <h1 style="font-size: bold">${
                                                  updateRoom.roomMembers[
                                                    updateRoom.roomMembers
                                                      .length - 1
                                                  ]._id
                                                }</h1> <p style="font-size: medium"> in Make a Payment option on our Website</p>
          ...
          const updateRoomMember = await Room.findOneAndUpdate(
            { $and: [{ _id: roomId }, { "roomMembers._id": memberId }] },
            {
              $set: {
            ...
            try {
              if (room) {
                ...
                if (isBooked === undefined) {
                  ...
                  const updatePreviousRoom = await Room.findByIdAndUpdate(
                    { _id: previousRoom },
                    {
                      $pull: {
