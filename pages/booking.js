...
const Booking = () => {
    ...
    if (phone == null && prevAttempt == null) {
      ...
      if (user.plab2Date !== undefined) {
        if (prevAttempt != null && phone != null) {
          console.log("Running");
          axios.put(`${process.env.url}/userData`, {
            user,
            prevAttempt,
            phone,
          });
      ...
      const res = await axios.post(`${process.env.url}/checkout_sessions`, {
        user,
        selectedCourse,
        coupon,
  ...
  const handleCoupon = async () => {
    ...
    const checkCoupon = await axios.post(
      `${process.env.url}/clientCoupon`,
      data,
    );
  ...
  return (
    <div className="flex items-center justify-center my-10 font-main">
      <div className="sm:h-slider sm:w-aboutWidth flex sm:shadow-2xl shadow-gray">
        <div className="hidden sm:flex relative w-1/3">
          ...
          <div className="absolute left-14 top-10 h-56 w-96">
            ...
            <p className="tracking-name absolute text-white font-bold bottom-5 left-7">
              TOM CRUISE
            </p>
            <p className="tracking-name absolute text-white font-bold bottom-5 right-7">
              00/00
            </p>
          </div>
        ...
        <div className=" sm:w-4/6">
          <div className="flex items-center justify-center">
            <h1 className="sm:ml-32 sm:mt-10 text-xl font-bold px-5">
              Welcome to Friends Academy Booking Area
            </h1>
          </div>
          <div className=" relative p-5 sm:p-0 sm:ml-28 mt-5 sm:mt-10 flex flex-col">
            ...
            <input
              type="number"
              min="0"
              placeholder="Previous Plab 2 Attempts"
              className=" sm:w-96 mt-10 border border-black py-4 px-3 rounded-md"
              value={prevAttempt}
              onChange={(e) => setPreviousAttempt(e.target.value)}
            />
            {user && !user.plab2Date ? (
              <input
                placeholder="Your Plab 2 Exam Date"
                className=" sm:w-96 mt-10 border border-black py-4 px-3 rounded-md"
                value={plab2}
            ...
            {calendar && (
              <div className="z-10 absolute bottom-40" id="calendar">
                <Calendar
            ...
            {emptyInputs == true ? (
              <Alert severity="error" className="mt-2 sm:w-96">
                Please fill the required fields!
              </Alert>
            ) : null}
            <div className="mt-10 flex sm:inline items-center justify-center">
              <button
                className="text-xl font-bold py-3 bg-green hover:bg-greenHover rounded-md w-2/4 mb-10 sm:mb-0"
                onClick={handlePay}
              >
                PAY £{selectedCourse && selectedCourse.price}
              </button>
              {isCoupon === true ? (
                <button
                  className="text-xl font-bold py-3 border rounded-md ml-5 sm:w-72 hover:bg-pink"
                  onClick={handleOpen}
                >
                  USE COUPON
                </button>
      ...
      >
        {apiRes === false ? (
          <div className="w-aboutPic h-52 my-5 flex flex-col items-center justify-center font-main">
            <h1 className="text-2xl font-bold">Enter Coupon Code</h1>
            <input
              type="text"
              className="w-96 font-bold px-5 py-3 rounded-lg mt-5 border focus:outline-green"
              onChange={(e) => setCouponCode(e.target.value)}
            />
            {invalidCoupon === true ? (
              <Alert
                sx={{ fontSize: "15px", fontWeight: "medium" }}
                variant="filled"
                severity="error"
                className="mt-2 w-96"
              >
            ...
            <button
              className="mt-5 bg-green rounded-md px-10 py-1 font-bold text-xl cursor-pointer hover:bg-greenHover"
