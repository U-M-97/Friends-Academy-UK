...
const RemoveCourse = () => {
  ...
  const calculatePayment = () => {
    ...
    if (muiCheckOutDate) {
      console.log(price);
      const totalAmount =
        Math.max(0, muiCheckOutDate.diff(muiCheckInDate, "day") + 1) * price;
  ...
  const handleCheckOutDate = () => {
    muiCheckOutDate &&
      setInputs((current) => {
        return {
          ...current,
          checkOut: dayjs(muiCheckOutDate),
        };
      });
    calculatePayment();
  };
  ...
  const handlePrevious = () => {
    if (
      displayColumn[displayColumn.length - 1].isSame(
        date
          .endOf("month")
          .set("hour", 0)
          .set("minute", 0)
  ...
  const handleSave = async () => {
    ...
    if (inputs.roomId != "") {
      ...
      if (
        ...
        res.data === "Member Updated Successfully"
      ) {
        handleClose();
        getRooms();
  ...
  const handleInputs = (column, room, member) => {
    ...
    setInputs((input) => ({
      ...
      status: member.paid == false ? "Unpaid" : "Paid",
      previousRoom: room._id,
    }));
  };
      ...
      <FullLayout>
        <div className="font-main">
          ...
          <div className="relative flex items-center justify-center py-5">
            <button
              className="mx-5 text-green hover:bg-greenHover hover:text-white rounded-full p-1"
            ...
            <button
              className="absolute bg-green rounded-md px-4 py-2 text-xl font-bold right-10 hover:bg-greenHover"
              onClick={handleOpen}
            >
              Add Booking
            </button>
          ...
          <div className="flex border-y border-gray">
            ...
            {displayColumn &&
              displayColumn.map((column) => {
                return (
                  <div
                    className="flex flex-col items-center justify-center py-2 w-20 border border-black"
                    key={column}
                  >
                  ...
                  <div className="flex border-b border-gray" key={room}>
                    ...
                    {displayColumn &&
                      displayColumn.map((column) => {
                        return (
                          <div
                            className="flex flex-col items-center h-20 justify-center w-20 border border-black cursor-pointer hover:bg-lightGray"
                            key={column}
                          >
                            {
                                      ...
                                      <>
                                        ...
                                        >
                                          <ConditionalRendering
                                            ...
                                            handleInputs={() =>
                                              handleInputs(column, room, member)
                                            }
                                            handleOpen={() =>
                                              handleOpen(column, room)
                                            }
                                          />
                                        </div>
                                      </>
        ...
        >
          <div className="w-dialog h-screen">
            <DialogTitle style={{ fontSize: "20px", fontWeight: "600" }}>
              Booking Menu
            </DialogTitle>
                            ...
                            {rooms &&
                              rooms.map((room) => (
                                <MenuItem
                                  key={room}
                                  value={room}
                                  onClick={() => handleRoomChanged(room)}
                                >
                                  {room.roomTitle}
                                </MenuItem>
                              ))}
            ...
            {apiRes === false ? (
              <DialogActions className=" border-lightGray border-t">
                ...
                <button
                  onClick={handleClose}
                  className="text-xl px-4 py-1 font-medium rounded-md"
                >
                  Cancel
                </button>
                {delButton === true ? (
                  <button
                    ...
                    className="text-xl text-white bg-red-600 px-4 py-1 font-medium hover:bg-red-500 rounded-md"
                  >
                    Delete
                  </button>
                ) : null}
                <button
                  onClick={handleSave}
                  className="text-xl bg-dashboard px-4 py-1 font-medium hover:bg-green rounded-md"
                >
                  Save
                </button>
              </DialogActions>
        ...
        <Modal open={open} onClose={handleCloseDelModal}>
          <Box className="absolute left-1/2 top-1/2 bg-white p-10 py-14 flex flex-col items-center justify-center -translate-x-1/2 -translate-y-1/2 font-main">
            <h1 className="text-xl font-bold">
              Are you sure you want to delete booking ? This cannot be undone
              and will permanently delete booking
            </h1>
            <div className="w-full flex items-center justify-center mt-5">
              {deleting === false ? (
                <button
                  className="mx-10 bg-red-500 w-32 h-9 text-lg text-white font-bold"
              ...
              <button
                className="mx-10 bg-green w-32 h-9 text-lg font-bold"
