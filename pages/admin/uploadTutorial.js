...
const VideoUpload = () => {
  ...
  const handleUpload = async () => {
    if (file == null) {
      ...
      const uploadComplete = await axios.post(
        `${process.env.url}/uploadVideo`,
        completedData,
      );
      ...
      const abortMultipart = await axios.post(
        `${process.env.url}/uploadVideo`,
        abortData,
      );
  ...
  const handleDelete = async () => {
    ...
    const deleteFile = await axios.delete(`${process.env.url}/files`, {
        ...
        <div className="font-main">
          <div className="flex items-center justify-center flex-col">
            <div className="border-2 flex items-center justify-center rounded-full pr-3">
              <label
                for="tutorial"
                className="bg-green px-10 py-2 text-lg font-bold rounded-l-full cursor-pointer"
              >
                Choose File
              </label>
            ...
            {uploading == false ? (
              <button
                className="mt-5 bg-green px-10 py-2 text-lg rounded-full font-bold"
                onClick={handleUpload}
              >
                Upload
              </button>
            ) : (
              <button className="mt-5 bg-green px-10 py-2 text-lg rounded-full font-bold">
                Uploading
              </button>
            )}
            {uploading === true ? (
              <div className="w-full bg-gray-200 rounded-full dark:bg-gray-700 mt-10">
                <div
                  className="bg-green text-lg font-medium text-black text-center p-0.5 leading-none rounded-full"
            ...
            {error === true ? (
              <Alert severity="error" className="w-2/4 mt-3">
                Please Add File
              </Alert>
            ) : null}
          </div>
          ...
          {files && files.length != 0
            ? files.map((item, index) => {
                console.log(index);
                if (
                  item.data.file.type === "png" ||
                  item.data.file.type === "jpg" ||
                  item.data.file.type === "jpeg"
                ) {
                  return (
                    <div
                      key={item}
                      className="border-2 border-pink rounded-lg flex-col px-10 py-5 mt-5 flex items-center"
                    >
                        ...
                        <div className="mt-5 flex items-center justify-center ">
                          {item.isEdit == true ? (
                            <div>
                              <input
                                value={input}
                                className="outline-none px-5 w-80 py-2 border-2 border-green rounded-lg"
                                onChange={(e) => setInput(e.target.value)}
                              />
                              <button
                                className="w-20 py-2 text-lg font-bold bg-green ml-5 rounded-lg"
                                onClick={() => handleSave(item.data.file)}
                              >
                                Save
                              </button>
                            </div>
                          ...
                          <div className="text-gray cursor-pointer ml-5">
                            {item.isEdit == false ? (
                              <EditIcon
                                onClick={() =>
                                  handleEdit(item.data.file.name, index)
                                }
                              />
                            ) : (
                              <CloseIcon
                      ...
                      <div className="w-full flex justify-end">
                        <button
                          className="bg-red-500 w-32 py-2 text-lg font-bold text-white rounded-lg"
                          onClick={() => handleOpen(item.data.file)}
                        >
                          Delete
                        </button>
                      </div>
                  ...
                  return (
                    <div
                      key={item}
                      className="w-full flex flex-col items-center mt-5 justify-center py-5 border-2 border-pink rounded-lg px-10"
                    >
                      <Player
                        fluid={false}
                        height={500}
