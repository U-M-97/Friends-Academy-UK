...
const Coupons = () => {
  ...
  const handleDelete = async (id) => {
    const delCoupon = await axios.delete(`${process.env.url}/coupon`, {
      data: id,
    });
  ...
  const handleIsEndDate = () => {
    if (isEndDate === true) {
      ...
      setInputs((input) => ({
        ...input,
        endDate: dayjs(date).format("DD/MM/YYY"),
      }));
  ...
  const handleCreate = async () => {
    if (
      ...
      inputs.discountOn != "" &&
      inputs.startDate != "" &&
      (isEndDate === true ? inputs.endDate != "" : inputs.endDate === null) &&
      (limitUsers === true
        ? inputs.totalUsers != ""
        : inputs.totalUsers === "unlimited")
    ) {
      setApiRes(true);
      setIsInputs(true);
    ...
    <ThemeProvider theme={theme}>
      <FullLayout>
        <div className="flex justify-between items-center mb-10">
          ...
          <button
            className="mr-40 bg-green w-52 flex items-center justify-center py-2 text-xl font-medium rounded-full"
          ...
          {apiRes === false ? (
            <div className="w-dialog h-dialogContainer">
                ...
                <div className="flex px-5 justify-between text-lg">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      placeholder="e.g, FRIENDSGIFT"
                      className="rounded-md border-2 border-slate-300 mt-2 w-72 px-2 py-1 hover:bg-slate-100 focus:border-green outline-none"
                      onChange={(e) =>
                        setInputs((input) => ({
                          ...input,
                ...
                <div className="flex items-center justify-between mt-5 px-5 text-lg">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      type="number"
                      min={0}
                      placeholder={inputs.discountType}
                      className="rounded-md border-2 border-slate-300 w-72 mt-2 px-2 py-1 hover:bg-slate-100 focus:border-green outline-none"
                      onChange={(e) =>
                        setInputs((input) => ({
                          ...input,
                  ...
                  <div className=" flex-1">
                    <p>Discount Type</p>
                    <select
                      className="w-72 mt-2  border-2 border-slate-300 rounded-md px-2 py-1 focus:border-green outline-none cursor-pointer"
                      onClick={(e) =>
                        setInputs((input) => ({
                          ...input,
                    ...
                    <div className="flex-1">
                      ...
                      >
                        {courses &&
                          courses.map((course) => {
                            return (
                              <option value={course.title} key={course}>
                                {course.title}
                              </option>
                            );
                          })}
                ...
                <div className="mt-10 px-5 text-lg flex flex-col">
                  ...
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <div className="flex items-center mt-5">
                      ...
                      <div
                        className={`ml-5 w-40 ${
                          isEndDate === false ? "invisible" : null
                        }`}
                      >
                        <DesktopDatePicker
                          label="End Date"
              ...
              <div className="mt-5 flex justify-end items-center text-lg font-medium my-5 mr-5">
                ...
                <button
                  className="py-1 px-8 rounded-full border-black border"
                  onClick={handleClose}
                >
                  Cancel
                </button>
                <button
                  className="py-1 px-8 rounded-full bg-green ml-5 border border-green hover:bg-greenHover"
