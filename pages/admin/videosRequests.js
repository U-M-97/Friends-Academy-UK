...
const VideosRequests = () => {
  ...
  const handleRemoveReq = async (id) => {
    const res = await axios.delete(`${process.env.url}/videosAdmin`, {
      params: { id, type: "remove request" },
    });
    getRequests();
  };
          ...
          req.requests.map((item) => {
            return (
              <div key={item} className="flex mt-2">
                ...
                <button
                  className="px-10 ml-10  bg-green"
                  onClick={() => handleApprove(item._id)}
                >
                  Approve
                </button>
                <button
                  className="px-10 ml-10  bg-red-600 text-white"
                  onClick={() => handleRemoveReq(item._id)}
                >
                  Remove
                </button>
              </div>
          ...
          req.access.map((item) => {
            return (
              <div key={item} className="flex mt-2">
                ...
                <p className="w-60 ml-10">
                  Started on {dayjs(item.time).format("DD/MM/YYYY")}
                </p>
                <button
                  className="px-10 ml-10  bg-red-600 text-white"
                  onClick={() => handleRemoveAccess(item.user._id)}
                >
                  Remove
                </button>
              </div>
