...
const Course = () => {
  ...
  useEffect(() => {
    if (course) {
      ...
      if (
        course.category === "Mini Mocks" ||
        course.category === "Personalised Sessions"
      ) {
    ...
    <div className="flex flex-col items-center font-main mb-10">
      <div className="sm:flex">
        ...
        <div className="sm:w-courseWidth p-5 sm:p-10 ">
          ...
          {course.startDate && (
            <div className="flex text-2xl items-center mt-5 ">
              ...
              <p className="ml-3">
                {dayjs(course.startDate).format("DD MMMM, YYYY")}
              </p>
            </div>
          )}
          <div className="mt-5 flex text-2xl">
          ...
          <pre className="mt-5 font-main text-xl whitespace-pre-wrap leading-10">
            {course.description}
          </pre>
        ...
        <div className="mt-10">
          {bookingType === "Book" ? (
            <button
              className="hidden sm:inline bg-green p-3 hover:bg-greenHover duration-300 rounded-md font-bold"
            ...
            <button
              className="bg-green p-3 hover:bg-greenHover duration-300 rounded-md font-bold"
      ...
      <div className="sm:w-aboutWidth flex flex-col items-center justify-center mb-10 mt-5 sm:mt-0">
        ...
        {course.startDate && (
          <div className="sm:w-courseWidth mt-5">
            <div className="flex flex-col sm:flex-row items-center justify-center mt-5">
              ...
              {course.endDate && (
                <p className="text-xl ml-2">
                  {dayjs(course.startDate).format("DD MMMM, YYYY")} -{" "}
                  {dayjs(course.endDate).format("DD MMMM, YYYY")}
                </p>
              )}
              {!course.endDate && (
                <p className="text-xl ml-2">
        ...
        {schedule &&
          schedule.map((item) => {
            return (
              <div
                className="flex mt-10 w-full sm:w-courseWidth items-center justify-between sm:text-xl border-b border-green pb-5"
                key={item}
              >
                <div>
                  <p>{item.day}</p>
      ...
      <div className="mt-14 flex flex-col items-center justify-center sm:w-courseWidth p-5 sm:p-0">
        ...
        <p className="text-xl mt-5">
          Bookings will open 420 days before the session starts.
        </p>
        <p className=" text-base text-justify  mt-5">
          {readMore == false ? text.slice(0, 300) : text}
          <span
      ...
      {bookingType === "Book" ? (
        <button
          className="bg-green py-3 px-5 rounded-md font-bold mt-10 hover:bg-greenHover duration-300"
          onClick={handleCourseCheck}
        >
          Book Now
        </button>
