    ...
    if (
      ...
      try {
        const res = await axios.post(
          `${process.env.url}/email/authEmail`,
          data,
        );
      ...
      <div className="flex h-signup w-aboutWidth shadow-lg shadow-gray rounded-2xl">
        ...
        <div className="relative flex-1 flex flex-col items-center bg-login justify-center rounded-r-2xl">
          <h1 className="font-bold text-2xl mt-12">
            Welcome to Friends Academy UK
          </h1>
          <input
            placeholder="First Name"
            name="firstName"
            value={inputs.firstName}
            className="p-3 w-96 rounded-sm outline-none hover:outline-pink focus:outline-pink mt-5"
          ...
          <input
            ...
            className="p-3 w-96 rounded-sm outline-none hover:outline-pink focus:outline-pink mt-2"
            onChange={handleClick}
          />
          <input
            placeholder="Email"
          ...
          />
          {emailFormat == false ? (
            <p className="text-red-600 font-bold">Email format is invalid</p>
          ) : null}
          <input
          ...
          />
          {calendar && (
            <div className="z-10 absolute top-50 left-50 " id="calendar">
              <Calendar
          ...
          <div
            className="flex items-center w-96 rounded-sm outline-none hover:outline-pink focus:outline-pink mt-2 bg-white cursor-text"
            onClick={() => setCalendar(true)}
          >
            <input
              placeholder="Your PLAB2 Date"
              className="p-3 outline:none h-full w-full focus:outline-none "
              value={plab2}
            />
          ...
          {userExist == true ? (
            <p className="text-red-600 font-bold">User already exists</p>
          ) : null}
          {loading === false ? (
            <button
              className="bg-green hover:bg-greenHover text-xl p-3 rounded-sm w-96 mt-2"
