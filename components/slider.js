  ...
  return (
    <div className="relative sm:h-slider overflow-hidden font-main">
      <div className="absolute z-30 text-white font-bold text-6xl w-screen flex items-center justify-center mt-28">
        <Typewriter
          onInit={(typewriter) => {
            typewriter.typeString("Leading PLAB-2 Instructors in Town").start();
          }}
          options={{
            autoStart: true,
            delay: 100,
      ...
      <div
        className="flex h-aboutPic sm:h-full w-picturesWrapper duration-1000 ease-in-out"
      ...
      <div
        className="hidden sm:h-20 sm:w-10 sm:flex sm:items-center sm:justify-center sm:absolute sm:top-56 sm:bottom-0 sm:left-20  sm:text-white sm:cursor-pointer sm:hover:text-green sm:z-20"
        onClick={() => handleArrow("left")}
      >
        <ArrowBackIosIcon className="scale-arrow ml-8" />
      </div>
      <div
        className="hidden sm:h-20 sm:w-10 sm:flex sm:items-center sm:justify-center sm:absolute sm:top-56 sm:bottom-0 sm:right-20  sm:text-white sm:cursor-pointer sm:hover:text-green sm:z-20"
