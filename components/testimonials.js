...
const Testimonials = () => {
  ...
  const [video, setVideo] = useState(
    "https://res.cloudinary.com/codillionaire/video/upload/v1672344896/friends-academy/Candidates_Reviews_-_Friends_Academy_Ltd_-_Manchester_video-converter.com_lreziu.mp4",
  );
  const videos = [
    {
  ...
  return (
    <div
      className="font-main flex flex-col justify-center items-center bg-servicesBG h-full  "
      id="reviews"
    >
      <div ref={header}>
        <motion.div
        ...
        >
          {reviews &&
            reviews.map((item) => {
              return (
                <SwiperSlide
                  className="mt-5 py-10 px-5 mb-5 bg-white"
                  key={item._id}
                >
      ...
      <Swiper
        className="hidden sm:flex w-aboutWidth mt-10"
        style={{
          "--swiper-navigation-color": "black",
        ...
        {videos.map((item) => {
          return (
            <SwiperSlide key={item}>
              <div
                className="mx-3 cursor-pointer border-4 border-green h-60 overflow-hidden bg-black"
      ...
      <Swiper
        className=" sm:hidden w-screen mt-10"
        style={{
          "--swiper-navigation-color": "black",
          "--swiper-navigation-size": "50px",
          "--swiper-navigation-margin": "50px",
