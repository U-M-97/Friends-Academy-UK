...
const Navbar = (props) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`font-main fixed top-0 flex flex-col h-full w-screen bg-white duration-700 z-50 sm:hidden ${
          open == false ? " -translate-x-full" : " translate-x"
        }`}
      >
        <div className="flex justify-end mt-5">
          <div
            className="bg-green p-2 h-10 w-10 mr-5 rounded-md"
            onClick={handleClick}
          >
        ...
        <div className="mt-10">
          <Link href="/">
            <a
              className="p-4 font-bold text-2xl border-b border-b-lightGray flex"
              onClick={handleClick}
            >
              HOME
            </a>
          ...
          <div className="p-4 font-bold text-2xl border-b border-b-lightGray">
            BLOG
          </div>
          <div className="p-4 font-bold text-2xl border-b border-b-lightGray">
            GALLERY
      ...
      <div className="hidden py-1 sm:flex font-main sm:justify-center">
        ...
        <ul className={`flex ${user ? "ml-20" : "ml-40"} items-center`}>
          <li
            className={`mr-8 duration-200 ${
              scrolled === true ? "hover:text-white" : "hover:text-green"
            }`}
          >
            <Link href="/">
          ...
          <li
            className={`mr-8 duration-200 cursor-pointer ${
              scrolled === true ? "hover:text-white" : "hover:text-green"
            }`}
            id="moreInformation"
          ...
          <div className="relative inline-block">
            {mouseEnter == true || isClicked == true ? (
              <div
                className="right-6 absolute w-52 top-2 z-30 bg-white"
                id="dropDown"
                onMouseEnter={() => setMouseEnter(true)}
                onMouseLeave={() => setMouseEnter(false)}
              >
          ...
          {user == null ? null : (
            <div
              className="mr-4 text-gray relative"
              onMouseEnter={() => setMouseEnterProfile(true)}
              onMouseLeave={() => setMouseEnterProfile(false)}
            >
              <div
                id="profileAvatar"
                className="border-2 border-black h-16 w-16 relative overflow-hidden rounded-full cursor-pointer"
              ...
              {isClickedProfile == true || mouseEnterProfile == true ? (
                <div
                  id="profileMenu"
                  className="flex flex-col justify-between absolute top-16 right-0 border-t-4 border-green bg-white z-40 w-96 text-black"
                >
                  <Link href="/profile">
                  ...
                  <div
                    className="border-b border-lightGray py-4 px-5  hover:bg-green hover:text-white duration-300 cursor-pointer"
                    onClick={logoutUser}
                  >
