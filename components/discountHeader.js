  ...
  useEffect(() => {
    coupons &&
      setOutputs((output) => ({
        ...
        discountOn: coupons[0].discountOn,
        discountType: coupons[0].discountType,
        endDate: coupons[0].endDate,
      }));
  }, []);
    ...
    <div className="font-main flex justify-between items-center border-t bg-saleHeader border-lightGray px-2 sm:px-7">
      {outputs && (
        <div className="flex items-center justify-center sm:w-11/12 sm:h-10">
          ...
          >
            {outputs.discount} {outputs.discountType} Discount on{" "}
            {outputs.discountOn} till {outputs.endDate}. Hurry up and Grab this
            limited offer right now
          </motion.p>
        </div>
      )}
